# Token Vesting Dashboard - Specification v1b
...
Based on Grumpy Carlos's review, the following changes have been made:

### Critical Fixes
1. **Fixed CreateVestingForm approval bug**: Now correctly uses `useDeployedContractInfo("TokenVesting")` to get the vesting contract address for token approval
2. **Added `startTime` validation**: Contract now rejects start times in the past to prevent accidental immediate vesting
3. **Added `NothingToRevoke` error**: Prevents wasteful revocations when schedule is fully vested

### Improvements Applied
1. Simplified the signature: `startTime` now defaults to `block.timestamp` (removed from function params)
2. Cached storage reads in `claim()` and `revokeVesting()` for gas optimization
3. Added proper loading states for owner check on admin page
4. Replaced non-null assertions with proper type filtering
5. Added SE-2 notifications instead of console.error
6. Created typed `VestingSchedule` interface for cleaner props
7. Added comment about client time usage in VestingProgressBar
8. Added Pausable pattern for emergency scenarios
9. Expanded test coverage with edge cases
...
**Key Changes from v1a:**
- Removed `_startTime` parameter - now always starts at `block.timestamp`
- Added `NothingToRevoke` error
- Added `Pausable` for emergency scenarios
- Cached `beneficiary` storage reads in `claim()` and `revokeVesting()`
...
export const CreateVestingForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notification.error(`Failed to create vesting schedule: ${message}`);
    }
  };
...
#### 6. Admin Page (vesting/admin/page.tsx) - Fixed with Loading State
...
const AdminPage = () => {
  ...
  const { writeContractAsync: revokeSchedule, isMining: isRevoking } = useScaffoldWriteContract("TokenVesting");
...
export const AdminScheduleRow = ({ scheduleId, onRevoke, isRevoking }: AdminScheduleRowProps) => {
  const { data: schedule } = useScaffoldReadContract({
    contractName: "TokenVesting",
    functionName: "getVestingSchedule",
    args: [scheduleId],
...
describe("TokenVesting", function () {
  ...
  describe("revokeVesting", function () {
    ...
    it("should allow beneficiary to claim vested portion after revocation", async function () {
      ...
      await tokenVesting.connect(beneficiary).claim(0);
      const balanceAfterFirstClaim = await vestingToken.balanceOf(beneficiary.address);
...
4. **Struct packing**: `uint64` timestamps pack efficiently with booleans
